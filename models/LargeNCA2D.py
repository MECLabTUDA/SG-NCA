...
def merge_ncas(base_nca: NCA2D, new_nca: NCA2D) -> NCA2D:
    ...
    assert isinstance(base_nca.batch_norm, nn.Identity)
    assert isinstance(new_nca.batch_norm, nn.Identity)
    assert base_nca.fire_rate == new_nca.fire_rate
    assert base_nca.num_steps == new_nca.num_steps
    merged_nca = NCA2D(num_channels=base_nca.num_channels + new_nca.num_channels, num_input_channels=base_nca.num_input_channels, 
                       num_classes=16, hidden_size=hidden_size0 + hidden_size1, fire_rate=base_nca.fire_rate, num_steps=base_nca.num_steps, use_norm=False).to(device)

    merged_nca.conv.weight = torch.nn.Parameter(torch.cat([base_nca.conv.weight, new_nca.conv.weight], dim=0))
    assert base_nca.conv.bias is None and new_nca.conv.bias is None, "Only conv layers without bias are supported."
    assert new_nca.conv.bias is None
    ...
    merged_nca.fc1.bias = torch.nn.Parameter(torch.cat([base_nca.fc1.bias, new_nca.fc1.bias], dim=0))
...
class LargeNCA2D(nn.Module):
    ...
    def _nca_update_merged(self, states: list[torch.Tensor]):
        state = torch.cat(states, dim=1)
        assert not hasattr(self, "new_nca"), "New NCA should have been merged into base NCA."
        assert self.base_nca.fire_rate == 1.0, "Merged NCA only supports fire_rate of 1.0 for now."

        if True:
            delta_state = self.base_nca.conv(state)
            delta_state = torch.cat([state, delta_state], dim=1)
            delta_state = self.base_nca.fc0(delta_state)
            delta_state = self.base_nca.batch_norm(delta_state)
            delta_state = F.relu(delta_state, inplace=True)
            delta_state = self.base_nca.fc1(delta_state)
            ...
            state = state[:, self.base_nca.num_input_channels:] + delta_state
            state = torch.cat([states[0][:, :self.base_nca.num_input_channels], state], dim=1)
        else:
            new_state = torch.ones_like(state)
            nca_cuda.nca2d_cuda(state, new_state, self.base_nca.conv.weight, self.base_nca.fc0.weight, self.base_nca.fc0.bias, self.base_nca.fc1.weight, self.base_nca.fc1.bias)
            state = new_state

        states = list(torch.split(state, [states[0].shape[1], states[1].shape[1]], dim=1))
        return states


    def make_state(self, x: torch.Tensor) -> list[torch.Tensor]:
        if hasattr(self, "new_nca"):
            ...
            state2 = torch.zeros(x.shape[0], self.num_new_channels, x.shape[2], x.shape[3], device=x.device)
            states = [torch.cat([x, state1], dim=1), state2]
        else:
            state1 = torch.zeros(x.shape[0], self.base_nca.num_channels - self.base_nca.num_input_channels - self.num_new_channels,
                        x.shape[2], x.shape[3], device=x.device)
            state2 = torch.zeros(x.shape[0], self.num_new_channels, x.shape[2], x.shape[3], device=x.device)
            states = [torch.cat([x, state1], dim=1), state2]
        return states
    ...
    def forward_internal(self, states: list[torch.Tensor]) -> list[torch.Tensor]:
        for _ in range(self.base_nca.num_steps):
            if hasattr(self, "new_nca"):
                ...
                states = self._nca_update_merged(states)
        return states
    ...
    def _init_new_nca(self, num_channels: int, num_new_classes: int,
        ...
        self.num_new_channels = self.new_nca.num_channels
