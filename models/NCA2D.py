...
class NCA2D(nn.Module):
    def __init__(self, num_channels: int, num_input_channels: int, num_classes: int,
        ...
        self.fc0 = nn.Conv2d(2 * num_channels, hidden_size, kernel_size=1, bias=not use_norm)
        self.fc1 = nn.Conv2d(hidden_size, num_channels - num_input_channels, kernel_size=1, bias=True)

        self.conv = nn.Conv2d(num_channels, num_channels, kernel_size=3, padding='same', 
                              padding_mode="reflect", groups=num_channels, bias=False)
        if use_norm:
